...
app.use((req, res, next) => {
  ...
  // set an empty form for all requests
  res.locals.form = {}
...
app.post(
  '/',
  (req, res, next) => {
    // upload the text (if there is one)
    const body = req.body
    if (!body.text || !body.text.length) {
      res.locals.err = 'Paste text missing (required)'
      res.render('index', { form: body })
      return
    }
    ...
    const id = zid(8)
    res.locals.id = id
    space.upload(id, body.text, (err, data) => {
      // data: {
      //   ETag: '"a56671ed1abf0616dc791292a090f367"',
      //   Location: 'https://paste-dev.sfo2.digitaloceanspaces.com/zcmcFtrw',
      //   key: 'zcmcFtrw',
      //   Key: 'zcmcFtrw',
      //   Bucket: 'paste-dev'
      // }
